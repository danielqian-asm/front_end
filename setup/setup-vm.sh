...
gcloud compute ssh $vmname --zone=$zone --command 'sudo mkdir -p /opt/app/new-repo' -q
gcloud compute scp --recurse *  $vmname:/opt/app/new-repo --zone=$zone -q
gcloud compute scp ./setup/start.sh  $vmname:~/start.sh --zone=$zone -q
gcloud compute ssh $vmname --zone=$zone --command 'sudo sh ./start.sh' -q
